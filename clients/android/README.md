...
The repository does not check in a Gradle wrapper yet; Android Studio supplies
the pinned Gradle tooling, or the commands above require `gradle` on `PATH`.
...
inference. `enqueueCaptureDerived` seals locally derived metadata plus its
source and derivation mode into the outbox without persisting raw frames. The
current MediaProjection adapter uses `privacy_gated_local_metadata`; a platform
AI worker may replace that derivation later without changing the envelope shape.
